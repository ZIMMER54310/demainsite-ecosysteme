# ============================================================
# DEMAINSITE ECOSYSTEME - VILLES PAR DEPARTEMENT V1.1
...
function Register-ProjectNote {
    ...
    $note = "A prevoir dans DemainSite Ecosysteme : gestion des villes / communes par departement, avec article WordPress obligatoire futur, medias lies, produits lies, filtres, score de completude et liaison SharePoint -> WordPress. Suppression automatique interdite."
    ...
    $values = @{
        ...
        "CodeProjetDSE" = $code
        "StatutProjetDSE" = "A construire / SharePoint d'abord"
        "NoteProjetDSE" = $note
    }
    if ($items.Count -gt 0) {
...
try {
    WTitle "DEMAINSITE ECOSYSTEME - VILLES PAR DEPARTEMENT V1.1"
    ...
    if ($choice -in @("OUI","YES","Y","O","ALL","TOUT","TOUS")) { $choice = "TOUS" }
    if ($choice -in @("STRUCTURE","STRUCT","S")) { $choice = "STRUCTURE" }
