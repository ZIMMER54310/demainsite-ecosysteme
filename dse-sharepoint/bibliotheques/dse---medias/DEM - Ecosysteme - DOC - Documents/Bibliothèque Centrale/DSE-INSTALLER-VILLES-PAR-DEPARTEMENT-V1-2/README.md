# DSE-INSTALLER-VILLES-PAR-DEPARTEMENT-V1.2
...
Dans la version V1.0, si l utilisateur saisissait `oui`, le script cherchait un departement nomme OUI.
La V1.1 corrige cela : `OUI`, `YES`, `ALL` et `TOUS` lancent maintenant l import de tous les departements.
...
Cette version est orientee PREMIERE INSTALLATION SHAREPOINT.

- `OUI`, `OK`, `1`, `STRUCTURE` = creation des listes par departement sans import massif.
- `54` ou `57` = creation de la liste du departement et import du departement choisi.
- `TOUS` = import complet de tous les departements.
- Correction de l'erreur `No -Identity has been provided` lors de la mise a jour de la note projet.

Objectif conseille maintenant : lancer `OUI` ou `STRUCTURE` pour creer toutes les sous-listes `Villes-01`, `Villes-02`, etc., puis tester un departement ensuite.
