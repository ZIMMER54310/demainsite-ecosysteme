...
# V1.1 - Villes + CodesPostaux + CodesPostauxVilles + recherche CSV automatique
# ============================================================
# Regles :
...
function Resolve-CsvPath {
    # 1) Chemin donne en parametre
    if ($CsvPath -and (Test-Path $CsvPath)) {
        Write-Host "CSV fourni en parametre : $CsvPath" -ForegroundColor Green
        return (Resolve-Path $CsvPath).Path
    }

    # 2) Dossier data du pack actuel
    ...
    if ($null -ne $csv) {
        Write-Host "CSV trouve dans le dossier data du pack : $($csv.FullName)" -ForegroundColor Green
        return $csv.FullName
    }

    # 3) Chemin probable retrouve dans le journal V1.1
    $knownCandidates = @(
    ...
    foreach ($candidate in $knownCandidates) {
        if (Test-Path $candidate) {
            Write-Host "CSV retrouve automatiquement : $candidate" -ForegroundColor Green
            return (Resolve-Path $candidate).Path
        }
    }

    # 4) Recherche limitee dans les dossiers probables, pas tout le disque
    $searchRoots = @()
    foreach ($folder in @("DemainSite", "Downloads", "Téléchargements", "OneDrive", "Documents")) {
        $path = Join-Path $env:USERPROFILE $folder
        if (Test-Path $path) { $searchRoots += $path }
    }

    foreach ($rootSearch in $searchRoots) {
        Write-Host "Recherche CSV dans : $rootSearch" -ForegroundColor Yellow
        $found = Get-ChildItem -Path $rootSearch -Recurse -File -ErrorAction SilentlyContinue |
            Where-Object {
        ...
        if ($null -ne $found) {
            Write-Host "CSV retrouve par recherche automatique : $($found.FullName)" -ForegroundColor Green
            return $found.FullName
        }
    }

    throw "Aucun CSV trouve automatiquement. Place le CSV dans le dossier data du pack ou relance avec -CsvPath 'chemin\fichier.csv'."
}
...
try {
    Write-Title "DEMAINSITE - REIMPORT VILLES COMPLETES V1.1"
